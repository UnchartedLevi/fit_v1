...
function Rings(){
 ...
 useFrame((state,delta)=>{if(!group.current)return;const mobile=size.width<=800;group.current.position.x=THREE.MathUtils.lerp(group.current.position.x,mobile?0:2.65,.05);group.current.position.y=THREE.MathUtils.lerp(group.current.position.y,mobile?.1:0,.05);group.current.rotation.x=THREE.MathUtils.lerp(group.current.rotation.x,pointer.current.y*.22,.04);group.current.rotation.y=THREE.MathUtils.lerp(group.current.rotation.y,pointer.current.x*.35,.04);group.current.rotation.z+=delta*.055;group.current.scale.setScalar(1+Math.sin(state.clock.elapsedTime*1.2)*.018)});
