...
function Rings(){
 ...
 useFrame((state,delta)=>{if(!group.current)return;group.current.position.x=THREE.MathUtils.lerp(group.current.position.x,size.width<=800?0:2.65,.05);group.current.rotation.x=THREE.MathUtils.lerp(group.current.rotation.x,pointer.current.y*.22,.04);group.current.rotation.y=THREE.MathUtils.lerp(group.current.rotation.y,pointer.current.x*.35,.04);group.current.rotation.z+=delta*.055;group.current.scale.setScalar(1+Math.sin(state.clock.elapsedTime*1.2)*.018)});
 return <group ref={group}>{rings.map((radius,i)=><mesh key={radius} rotation={[Math.PI/2+i*.035,i*.08,0]}><torusGeometry args={[radius,i===3?.025:.012,12,160]}/><meshBasicMaterial color={i===3?"#d9ff43":"#737373"} transparent opacity={i===3?.9:.34-i*.025}/></mesh>)}</group>
}
function FlowField(){
 ...
 return <points ref={points}><bufferGeometry><bufferAttribute attach="attributes-position" args={[positions,3]}/></bufferGeometry><pointsMaterial color="#d9ff43" size={.025} transparent opacity={.42} sizeAttenuation/></points>
}
export function HeroScene(){return <div className="hero-scene" aria-hidden="true"><Canvas camera={{position:[0,0,8.7],fov:48}} dpr={[1,1.5]} gl={{antialias:true,alpha:true,powerPreference:"high-performance"}}><FlowField/><Sparkles count={42} scale={[9,7,3]} size={1.25} speed={.28} opacity={.24} color="#fff"/><Float speed={1.15} rotationIntensity={.12} floatIntensity={.32}><Rings/></Float></Canvas><div className="hero-scene-label"><span>EST. 2024</span><strong>FITS<br/>FOR LIFE</strong></div><div className="hero-scene-scan"/></div>}
